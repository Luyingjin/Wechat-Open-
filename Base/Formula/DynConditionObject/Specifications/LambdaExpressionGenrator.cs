...
namespace Formula.DynConditionObject
{
    ...
    {
        #region 静态私有键值集合，记录各种不同运算符转化为 Lambda运算符
        ...
        static List<ITransFormProvider> _transformProviders;
        ...
        private static Expression GetExpressoinBody<T>(ParameterExpression param, IEnumerable<ConditionItem> items)
        {
            ...
            if (andList.Count() != 0)
            {
                list.Add(GetGroupExpression<T>(param, andList, Expression.AndAlso));
            }
            ...
            {
                if (group.Count() != 0)
                    list.Add(GetGroupExpression<T>(param, group, Expression.OrElse));
            }
        ...
        public static Expression<Func<T, bool>> GenerateExpression<T>(List<ConditionItem> items)
        {
