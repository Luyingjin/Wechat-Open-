...
{
    public class BlockData : BasePage
    {
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            switch (reqAction)
            {
                ...
                case "SaveOrder"://保存页面布局
                    ...
                    Block.SaveGetBlocks(UserID, orders, this.Request["BlockType"], this.Request["TemplateId"]);
                    break;
                case "GetAllBlock":
                    content = Block.GetAllBlockNames(UserID, this.Request["BlockType"], this.Request["TemplateId"]);
                    break;
                case "GetOneNew":
                    content = Block.GetOneBlockHtmls(this.Request["BlockId"]);
                    Block.UpdateAfterAddNewOneBlock(UserID, this.Request["BlockId"], this.Request["TemplateId"]);
                    break;
                case "DeleteBlock":
                    Block.DeleteBlockFromTemplate(UserID, this.Request["BlockId"], this.Request["TemplateId"]);
                    break;
                case "ChangeColumns":
                    ...
                    Block.SaveChangeColumns(UserID, columns, templateString, this.Request["BlockType"], this.Request["TemplateId"], layout1, layout2, layout3, layout4);
                    break;
                case "ChangeWidth":
                    ...
                    Block.ChangeColumnsWidth(UserID, columns1, this.Request["BlockType"], this.Request["TemplateId"], layout11, layout21, layout31, layout41);
                    break;
                case "Reset":
                    Block.ResetBlocks(UserID, this.Request["TemplateId"]);
                    break;
                #endregion
        ...
        private string GetBlockContent(Block bl)
        {
            try
            {
                //特殊Block直接输出html
        ...
        private string GetAuditedTaskContent(Block bl)
        {
            string sql = @"select FormInstanceID
,S_WF_InsTaskExec.ID
...
order by S_WF_InsTaskExec.ExecTime desc";
        ...
        private string GetAuditTaskContent(Block bl)
        {
            string preYear = DateTime.Now.AddYears(-1).ToString("yyyy-MM-dd");

            string sql = @"select S_WF_InsTaskExec.ID
,S_WF_InsTask.TaskName
,case when  S_WF_DefSubForm.ID is null then S_WF_InsDefFlow.FormUrl else S_WF_DefSubForm.FormUrl end as FormUrl
,case when  S_WF_DefSubForm.ID is null then S_WF_InsDefFlow.FormWidth else S_WF_DefSubForm.FormWidth end as FormWidth
,case when  S_WF_DefSubForm.ID is null then S_WF_InsDefFlow.FormHeight else S_WF_DefSubForm.FormHeight end as FormHeight
...
order by S_WF_InsTask.Urgency desc,S_WF_InsTaskExec.CreateTime desc";
        ...
        private string GetDesignTaskContent(Block bl)
        {
        ...
        private DataTable bl_GetServiceData(string blockKey, int? count)
        {
            ...
            if (dtCatlog != null && dtCatlog.Rows.Count > 0)
            {
                ...
                if (Config.Constant.IsOracleDb)
                {
                    ...
                    dt = sqlHelper.ExecuteDataTable("select * from S_I_PublicInformation where CatalogId = '" + dtCatlog.Rows[0]["ID"].ToString() + "' and (ExpiresTime is null or to_char(ExpiresTime,'yyyy-MM-dd') >= '" + DateTime.Now.ToString("yyyy-MM-dd") + "') " + whereSql + " ORDER BY Important DESC,Urgency DESC, CreateTime DESC", 0, iCount, CommandType.Text);
                }
                else
                {
                    string whereReceiveDeptId = string.Empty;
        ...
        private string GetFrameContent(Block bl)
        {
            if (bl != null)
