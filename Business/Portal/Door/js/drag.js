...
DragDrop.prototype = {
    initialize: function(elementHeader_id, element_id) {
        ...
        this.header = _$(elementHeader.id);
...
DragUtil.getSortIndex = function(){
	...
	for(var i = 0; i < col_array.length ; i++){
		var b=false;
		var childs = jQuery("#" + col_array[i]).children(".drag_div:not(.no_drag)");
...
var initDrag = function() {
    ...
    var tmpElements = jQuery(".drag_div");
...
// 结束拖拽
var DRAG_URL = "/Portal/Door/BlockData.aspx";
function end_Drag(){
	if(this._afterDrag()){
		// 在这可以做一些拖拽成功后的事，比如 Ajax 通知服务器端修改坐标，以便下次用户进来时位置不变
	    var str = DragUtil.getSortIndex();
	    var par = 'Param=SaveOrder&Orders=' + str + '&BlockType=' + _blockType + "&TemplateId=" + _templateId;
	    saveDrag(DRAG_URL, par);
		//document.getElementById(this.elm.id.replace("drag_","drag_refresh_img_")).click()
	    //setTimeout('document.getElementById("'+this.elm.id.replace("drag_","drag_refresh_img_")+'").click()',"500");
	}
	return true;
}
