...
mtDropDown._activate = null;

function BuildMenu(ms, item) {
    var key = item.actionkey;
    var menu = ms.addMenu(document.getElementById(key));
    if (!menu)
        return;
    var items = item.children;
    if (items != null) {
        for (var i = 0; i < items.length; i++) {
            createsubMenu(menu, items[i]);
        }
    }
}

function createsubMenu(parentMenu, item) {
    var name = item.name;
    var linkUrl = item.linkurl;
    var menuItem = parentMenu.addItem(name, linkUrl);
    var children = item.children;
    if (children != null) {
        if (children.length > 0)
            var subMenu = parentMenu.addMenu(menuItem);
        for (var i = 0; i < children.length; i++) {
            createsubMenu(subMenu, children[i]);
        }
    }
}
...
if (oh != 2) {
    ...
    elmCache["clip"].style.width = ow + mtDropDown.shadowSize + 2 + "px";
    elmCache["clip"].style.height = "0px";

    elmCache["content"].style.width = ow + mtDropDown.shadowSize + "px";
    elmCache["content"].style.height = "0px";
...
var _ohHeight = oh - (mtDropDown.shadowOffset - mtDropDown.shadowSize);
if (_ohHeight < 0) _ohHeight = 0;
elmCache["shadowRight"].style.height = _ohHeight + "px";
elmCache["shadowRight"].style.backgroundColor = mtDropDown.shadowColor;


var _ohWidth = ow - mtDropDown.shadowOffset;
if (_ohWidth < 0) _ohWidth = 0;
elmCache["shadowBottom"].style.top = oh + "px";
elmCache["shadowBottom"].style.width = _ohWidth + "px";
...
if (!animating) {
    if (_this.items[this._index].url) {
        menuclick(_this.items[this._index].text, _this.items[this._index].url,false,_this.items[this._index]);
    }
}
}
...
sText + '</td><td width="5" style="' + sPaddingRight + '">' + 
'<img src="' + sDingbat + '" width="5" height="5"></td></tr>';
}
}
