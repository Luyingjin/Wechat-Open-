...
{
    public class MpMediaImageController : BaseController<MpMediaImage>
    {
        ...
        public override JsonResult GetList(MvcAdapter.QueryBuilder qb)
        {
            ...
            var result = entities.Set<MpMediaImage>().Where(c => c.MpID == mpid && c.IsDelete == 0).WhereToGridData(qb);
            return Json(result);
        }

        public JsonResult AddFile()
        {
            List<MpMediaImage> list = new List<MpMediaImage>();
            var mpid = GetQueryString("MpID");
            ...
            foreach (var fn in filename.Split(',').Where(c => !string.IsNullOrEmpty(c)))
            {
                ...
                var entity = GetEntity<MpMediaImage>("");
                ...
                entities.Set<MpMediaImage>().Add(entity);
                list.Add(entity);
            }
        ...
        public override JsonResult Delete()
        {
            ...
            var mpid = GetQueryString("MpID");
            var etys = entities.Set<MpMediaImage>().Where(c => IDs.Contains(c.ID) && c.IsDelete == 0).ToList();
            var wxFO = Formula.FormulaHelper.CreateFO<WxFO>();
