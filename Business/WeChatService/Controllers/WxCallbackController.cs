...
using System.IO;
using WeChat.Logic.Service.MessageHandlers.CustomMessageHandler;
...
{
    public class WxCallbackController : BaseController
    {
        ...
        /// </summary>
        [HttpGet]
        public ActionResult CommonPost(PostModel postModel, string echostr)
        {
            ...
            {
                return Content("failed:" + postModel.Signature + "," + Senparc.Weixin.MP.CheckSignature.GetSignature(postModel.Timestamp, postModel.Nonce, postModel.Token) + "。" +
        ...
        /// </summary>
        [HttpGet]
        public ActionResult CommonMiniPost(PostModel postModel, string echostr)
        {
            var account = GetAccount(GetQueryString("MpID"));
            if (account == null)
            {
        ...
        /// </summary>
        [HttpPost]
        public ActionResult CommonMiniPost(PostModel postModel)
        {
            var account = GetAccount(GetQueryString("MpID"));
            if (account == null)
            {
                LogWriter.Info(string.Format("微信回调错误：找不到MpID为{0}的公众号", GetQueryString("MpID")));
                return new WeixinResult("未添加公众号！");//v0.8+
            }
            postModel.Token = account.Token;
            postModel.EncodingAESKey = account.EncodingAESKey;//根据自己后台的设置保持一致
            ...
            if (!CheckSignature.Check(postModel.Signature, postModel.Timestamp, postModel.Nonce, postModel.Token))
            {
                LogWriter.Info(string.Format("MpID为{0}的微信回调参数错误！", GetQueryString("MpID")));
                //return Content("参数错误！");//v0.7-
                return new WeixinResult("参数错误！");//v0.8+
            }

            var messageHandler = new CustomMessageHandler(Request.InputStream, postModel, account, entities, 10);
